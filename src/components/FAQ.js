...
const FAQ = ({ Data, isPage }) => {
  ...
  const [visibleFaqs, setVisibleFaqss] = useState(3);
  const totalFaqss = Data?.length;

  const loadMoreInsights = () => {
    setVisibleFaqss((prevVisibleFaqss) => prevVisibleFaqss + 3);
  };
  const Heading = isPage ? "h1" : "h3";
  return (
    <div className="flex w-full bg-bgURL py-8 sm:py-16 px-4 sm:px-16 bg-greenBg max-width">
      <div className="flex w-full flex-col justify-start items-center h-auto py-6 sm:py-8 sm:px-16">
        <Heading className="font-benzgrotesk text-black font76-16 pb-8 sm:pb-20 font-extrabold whitespace-nowrap">
          FAQS
        </Heading>
        {/* map over faqs */}
        {Data?.slice(0, visibleFaqs).map((faq, index) => (
          <div key={index} className="flex flex-col w-full">
            <hr className="h-px relative w-full opacity-30 bg-black border-0 mb-16 dark:bg-black" />
            <div className="flex justify-between pb-16">
              <p className="font14-32 font-normal uppercase">{index + 1}</p>
              <div className="font14-32 w-5/6 sm:w-4/5">
                <div className="group" onClick={() => handleToggle(index)}>
                  <summary
                    className={`list-none cursor-pointer flex flex-wrap items-center justify-between svgHover font14-32 font-normal uppercase`}>
                    <div className="w-4/5 mb-4 hover:scale-105 transition-all duration-500">
                      {faq.question}
                    </div>
                    <div
                      className={`hidden sm:block relative h-[15px] sm:h-[35px] w-[15px] sm:w-[40px] ${
                        activeIndex === index
                      ...
                      <svg
                        width="41"
                        height="36"
                        className="h-[15px] sm:h-[35px] w-[60px] sm:w-[50px]"
                        viewBox="0 0 41 36"
                        fill="none"
                  ...
                  <div
                    className={`overflow-hidden w-4/5 transition-[max-height] duration-500 ease-in ${
        ...
        {visibleFaqs < totalFaqss && (
          <div className="flex justify-center text-base pb-4 sm:pb-0">
            <div
              className="flex justify-center items-center font-glancyrMedium cursor-pointer w-fit h-10 p-4 pb-4 bg-greenBg hover:bg-black text-black hover:text-greenBg border border-black hover:border-greenBg rounded-full hover:scale-125 transition-all duration-500"
